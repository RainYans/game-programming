...
/// throw an item and HIT the dummies. Mandatory (no Skip). Drives a TutorialBanner + TutorialArrow
/// + a real TargetMarker ring; spawns/clears via BattleManager's sandbox hooks. On completion it
/// marks the tutorial done and returns to the farm. Positions are offsets from THIS object (place
/// it at the arena centre) so the whole layout is editable in the Inspector.
public class CombatTutorialController : MonoBehaviour
{
    ...
    private IEnumerator Run()
    {
        ...
        // 4) COMMAND two allies onto a dummy
        ...
        yield return new WaitUntil(() =>
            commandController != null && commandController.CommandCount > cmdBase && (cd == null || !cd.IsAlive));
        yield return Cheer();

        // 5) ITEMS — clear allies, throw and HIT the dummies
        ...
        yield return new WaitUntil(() => commandController != null && commandController.ItemsHitCount > hitBase);
        yield return Cheer();
        ...
        banner.Finish("Training complete! Heading back to the farm...");
