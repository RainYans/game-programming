...
public class GameConfig : ScriptableObject
{
    ...
    [Header("Lab (strain upgrades)")]
    [Tooltip("Resources for a strain's first upgrade; each further level costs labCostStep more.")]
    [Min(0)] public int labBaseCost = 60;
    [Min(0)] public int labCostStep = 60;
    [Tooltip("Highest upgrade level a strain can reach at the Lab.")]
    [Min(0)] public int labMaxLevel = 3;
    [Tooltip("Stat gain per upgrade level (0.12 = +12% attack & max HP each level).")]
    [Min(0f)] public float labStatStepPct = 0.12f;

    /// Resource cost to go from `currentLevel` to the next.
    public int LabCostFor(int currentLevel) => labBaseCost + labCostStep * Mathf.Max(0, currentLevel);
    /// Stat multiplier for a strain at `level` (1 = unupgraded).
    public float LabMultiplier(int level) => 1f + labStatStepPct * Mathf.Max(0, level);
