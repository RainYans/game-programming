...
public class DeployPanel : MonoBehaviour
{
    ...
    private void Deploy()
    {
        ...
        foreach (string uid in selected)
        {
            ...
            float upg = labManager != null ? labManager.MultiplierFor(unit.strainId) : 1f;
            squad.Add(new BattleHandoff.DeployedUnit { uid = uid, data = data, damageMultiplier = dealt, damageTakenMultiplier = taken, upgradeMultiplier = upg });
        }
