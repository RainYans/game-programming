...
public class BattleManager : MonoBehaviour
{
    ...
    private void SpawnSquad(Transform at)
    {
        if (BattleHandoff.HasDeployment)
        {
            foreach (BattleHandoff.DeployedUnit u in BattleHandoff.Squad)
                SpawnAgent(u.data, Team.Player, at, u.uid, u.damageMultiplier, u.damageTakenMultiplier, u.upgradeMultiplier);
        }
        else
        {
    ...
    private void SpawnAgent(ZombieData data, Team team, Transform at, string uid,
        float damageMultiplier = 1f, float damageTakenMultiplier = 1f, float upgradeMultiplier = 1f)
    {
        if (data == null) return;
        ...
        go.transform.localScale = Vector3.one * (agentScale * Mathf.Max(0.1f, data.displayScale));
